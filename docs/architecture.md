...
| `book_sources` | One row per (book, source_type, source_instance_id, source_bucket) — `source_bucket` is `'primary'`, or `'owned'`/`'shared'` for a Hardcover book's optional second row (see [docs/sync.md](sync.md)'s Hardcover Owned-List Import section) — stores external IDs, ISBNs, slugs, cross-system reference IDs, and Chaptarr monitored/has_file flags |
